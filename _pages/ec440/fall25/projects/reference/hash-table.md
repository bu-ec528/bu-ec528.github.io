---
title: ""
permalink: /EC440/fall25/projects/reference/hash_table
...
* <span style="color:CornFlowerBlue;">**Type: struct hash**</span>
...
* <span style="color:CornFlowerBlue;">**Type: struct hash\_elem**</span>
...
* <span style="color:CornFlowerBlue;">**Macro: type \*hash\_entry (struct hash\_elem \*elem, type, member)**</span>
...
Each hash table element must contain a key, that is, data that identifies and distinguishes elements, which must be _unique_ among elements in the hash table. (Elements may also contain non-key data that need not be unique.)
...
* <span style="color:CornFlowerBlue;">**Type: unsigned hash\_hash\_func (const struct hash\_elem \*element, void \*aux)**</span>
...
* <span style="color:CornFlowerBlue;">**Function: unsigned hash\_bytes (const void \*buf, size\_t \*size)**</span>
...
* <span style="color:CornFlowerBlue;">**Function: unsigned hash\_string (const char \*s**</span>**)**
  * **Returns a hash of null-terminated string&#x20;**_**s**_**.**
* <span style="color:CornFlowerBlue;">**Function: unsigned hash\_int (int i)**</span>
...
* <span style="color:CornFlowerBlue;">**Type: bool hash\_less\_func (const struct hash\_elem \*a, const struct hash\_elem \*b, void \*aux)**</span>
  ...
  * See section [Auxiliary Data](hash-table.md#6auxiliary-data), for an explanation of aux.
  * See section [Hash Table Example](hash-table.md#5hash-table-example), for hash and comparison function examples.

### 1.3 Hash Action Function
...
* <span style="color:CornFlowerBlue;">**Type: void hash\_action\_func (struct hash\_elem \*element, void \*aux)**</span>
  * **Performs some kind of action, chosen by the caller, on&#x20;**_**element**_**.**
  * See section [Auxiliary Data](hash-table.md#6auxiliary-data), for an explanation of _aux_.

## 2 Basic Functions

These functions _create_, _destroy_, and _inspect_ hash tables.

* <span style="color:CornFlowerBlue;">**Function: bool hash\_init (struct hash \*hash, hash\_hash\_func \*hash\_func, hash\_less\_func \*less\_func, void \*aux)**</span>
  * Initializes _hash_ as a hash table with _hash\_func_ as hash functi
  on, _less\_func_ as comparison function, and _aux_ as auxiliary data.
...
* <span style="color:CornFlowerBlue;">**Function: void hash\_clear (struct hash \*hash, hash\_action\_func \*action)**</span>
...
* <span style="color:CornFlowerBlue;">**Function: void hash\_destroy (struct hash \*hash, hash\_action\_func \*action)**</span>
  * **If&#x20;**_**action**_ **is non-null, calls it for each element in the** _**hash**_, with the same semantics as a call to `hash_clear()`.
  * Then, frees the memory held by _hash_.
  * Afterward, _hash_ must not be passed to any hash table function, absent an intervening call to `hash_init()`.
* <span style="color:CornFlowerBlue;">**Function: size\_t hash\_size (struct hash \*hash)**</span>
  *  Returns the number of elements currently stored in _hash_. 
* <span style="color:CornFlowerBlue;">**Function: bool hash\_empty (struct hash \*hash)**</span>
...
* <span style="color:CornFlowerBlue;">**Function: struct hash\_elem \*hash\_insert (struct hash \*hash, struct hash\_elem \*element)**</span>
...
* <span style="color:CornFlowerBlue;">**Function: struct hash\_elem \*hash\_replace (struct hash \*hash, struct hash\_elem \*element)**</span>
...
These functions allow iterating through the elements in a hash table. Two interfaces are supplied. The first requires writing and supplying a _**hash\_action\_func**_ to act on each element (see section [Data Types](hash-table.md#1data-types)).

* <span style="color:CornFlowerBlue;">**Function: void hash\_apply (struct hash \*hash, hash\_action\_func \*action)**</span>
  * **Calls&#x20;**_**action**_**&#x20;once for each element in&#x20;**_**hash**_**,&#x20;**<span style="color:Crimson;">**in arbitrary order**</span>**.**
...
* <span style="color:CornFlowerBlue;">**Type: struct hash\_iterator**</span>
...
* <span style="color:CornFlowerBlue;">**Function: void hash\_first (struct hash\_iterator \*iterator, struct hash \*hash)**</span>
  * **Initializes&#x20;**_**iterator**_ **to just before the first element in** **hash.**
* <span style="color:CornFlowerBlue;">**Function: struct hash\_elem \*hash\_next (struct hash\_iterator \*iterator)**</span>
...
* <span style="color:CornFlowerBlue;">**Function: struct hash\_elem \*hash\_cur (struct hash\_iterator \*iterator)**</span>
...
(The use of `UNUSED` in these functions' prototypes suppresses a warning that _aux_ is unused. See section [Function and Parameter Attributes](../test.md#function-and-parameter-attributes), for information about `UNUSED`)
...
`struct page` is allocated as a local variable here on the assumption that it is fairly small. Large structures should not be allocated as local variables. See section [Struct thread](./thread.md#1-struct-thread), for more information.
